...
import com.example.yang.jingqi.Utils.DateUtil;
import com.example.yang.jingqi.Utils.ScreenUtil;
import com.example.yang.jingqi.Utils.TextUtil;
    ...
    private void initView() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                String time;
                if (mMode == MODE.YMDHM)
                    time = DateUtil.format(selectedCalender.getTime(), FORMAT_YMDHM);
                else
                    time = DateUtil.format(selectedCalender.getTime(), FORMAT_YMD);
                handler.handle(time);
                seletorDialog.dismiss();
            }
        });
...
//        }
    ...
    private void loadComponent() {
        ...
        int myear = selectedCalender.get(Calendar.YEAR);
        int mmonth = selectedCalender.get(Calendar.MONTH);
        int mday = selectedCalender.get(Calendar.DAY_OF_MONTH);
        int mhour = selectedCalender.get(Calendar.HOUR_OF_DAY);
        int mminute = selectedCalender.get(Calendar.MINUTE);
        for (int i = 0; i < year.size(); i++) {
            if ((myear + "").equals(year.get(i))) {
                year_pv.setSelected(i);
            }
        }
        for (int i = 0; i < month.size(); i++) {
            if (fomatTimeUnit(mmonth + 1).equals(month.get(i))) {
                month_pv.setSelected(i);
            }
        }
        for (int i = 0; i < day.size(); i++) {
            if ((mday + "").equals(day.get(i))) {
                day_pv.setSelected(i);
            }
        }
        for (int i = 0; i < hour.size(); i++) {
            if ((mhour + "").equals(hour.get(i))) {
                hour_pv.setSelected(i);
            }
        }
        for (int i = 0; i < minute.size(); i++) {
            if ((mminute + "").equals(minute.get(i))) {
